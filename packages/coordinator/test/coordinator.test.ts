...
describe("heartbeat coordinator", () => {
  it("rejects a reader credential on write routes", async () => {
    const response = await invoke(env(), "POST", "/leases", { accountFingerprint: "abc", ttlSeconds: 10 }, {
  ...
  it("creates, renews, and reads normalized status without listing raw leases", async () => {
    const writer = env();
    const created = await invoke(writer, "POST", "/leases", {
    ...
    const renewed = await invoke(writer, "POST", "/leases/fp_shared/renew", { ttlSeconds: 20 }, {
      authorization: "Bearer writer-secret",
    });
  ...
  it("returns inactive after expiry and deletes the row", async () => {
    ...
    await invoke(runtime, "POST", "/leases", { accountFingerprint: "fp", ttlSeconds: 5 }, {
      authorization: "Bearer writer-secret",
    });
  ...
  it("rejects malformed input and oversize TTLs", async () => {
    const runtime = env();
    const bad = await invoke(runtime, "POST", "/leases", { accountFingerprint: "fp", ttlSeconds: 999 }, {
      authorization: "Bearer writer-secret",
    });
    expect(bad.status).toBe(400);
    const missing = await invoke(runtime, "POST", "/leases", { ttlSeconds: 10 }, {
      authorization: "Bearer writer-secret",
    });
  ...
  it("releases a lease", async () => {
    const runtime = env();
    await invoke(runtime, "POST", "/leases", { accountFingerprint: "fp", ttlSeconds: 10 }, {
      authorization: "Bearer writer-secret",
    });
