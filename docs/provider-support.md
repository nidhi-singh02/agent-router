...
| Codex `codex`        | `--model` only. No reasoning-effort flag in `codex --help`.                                                                    | Local-session: `~/.codex/statusline-quota-cache.json` `{ weekly_left, at }` (percent remaining, Unix seconds, 15-minute max age). | The router does not write this file.               |
